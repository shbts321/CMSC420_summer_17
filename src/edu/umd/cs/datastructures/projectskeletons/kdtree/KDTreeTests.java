package edu.umd.cs.datastructures.projectskeletons.kdtree;

import org.junit.After;
import org.junit.Before;
import org.junit.Test;

import edu.umd.cs.datastructures.projectskeletons.kdtree.utils.KDPoint;
import edu.umd.cs.datastructures.projectskeletons.kdtree.utils.KNNComparator;

import java.util.*;

import static org.junit.Assert.*;
import static edu.umd.cs.datastructures.projectskeletons.kdtree.utils.KDPoint.distance;
...
public class KDTreeTests {
    ...
    private static final int DEFAULT_BPQ_CAPACITY = 10;
    private static final int DEFAULT_BPQ_ITER = 500;
    private static final int MAX_ELEMENT_PRIORITY = 1000;

    //private static final double EPSILON = Math.pow(10,  -12);
    private static final double EPSILON = Math.pow(10,  -12);
    private static final int MAX_KDTREE_ITER = 200;
     ...
     * @param neighbors A {@link java.util.List} containing the ground truth response to the query.
    ...
    public void setUp(){
        // Set -up BPQ part
        try {
            people = new BoundedPriorityQueue<>(DEFAULT_BPQ_CAPACITY);
        }catch(Throwable t){
            fail("During setUp(), we caught a " + t.getClass().getSimpleName() +
                    " with message: " + t.getMessage() + " when creating a BPQ with the default capacity of:" +
                    DEFAULT_BPQ_CAPACITY + ".");
        }
        ...
        // And don't forget the random number generator.
    ...
    @Test
    public void testBPQInvalidPrioritySupplied(){

        // First, test that for a bunch of invalid priorities,
        // an exception is thrown.
        for(int i = 0; i < DEFAULT_BPQ_ITER; i++){
            int randNegInt = -r.nextInt(MAX_ELEMENT_PRIORITY); // 0, -1, ..., -MAX_ELEMENT_PRIORITY + 1
            try {
                new BoundedPriorityQueue<>(randNegInt);
            } catch(RuntimeException ignored){} // Do nothing, let the loop go
            catch(Throwable t){
        ...
        for(int i = 0; i < DEFAULT_BPQ_ITER; i++){
            int randInt = r.nextInt(MAX_ELEMENT_PRIORITY); // 0, 1, ..., MAX_ELEMENT_PRIORITY - 1
            try {
                new BoundedPriorityQueue<>(randInt);
            }
            catch(Throwable t){
                fail("When creating BPQ # " + i + " with priority " + randInt +
                        ", we caught a " + t.getClass().getSimpleName() + " with message: " +
                        t.getMessage() + ".");
            }
        }
    }
    ...
    public void testBPQEnqueue() {
        ...
        int priority = r.nextInt(MAX_ELEMENT_PRIORITY);
        try {
            people.enqueue("Person " + DEFAULT_BPQ_CAPACITY, priority);
        }catch(Throwable t){
            fail("Caught a " + t.getClass().getSimpleName() + " with message: " + t.getMessage() +
                    " when inserting element \"Person " + DEFAULT_BPQ_CAPACITY + "\"" +
        ...
        assertEquals("Inserting an element beyond a BPQ's capacity should not expand the queue, "
                + "which is what happened when inserting element \"Person " + DEFAULT_BPQ_CAPACITY + "\"" +
                " with priority " +	priority + ".",	DEFAULT_BPQ_CAPACITY, people.size());

        // (3) Keep on doing that DEFAULT_BPQ_ITER - many times, always making sure the size
        // remains the same.
        for(int i = 0; i < DEFAULT_BPQ_ITER; i++){
            priority = r.nextInt(MAX_ELEMENT_PRIORITY);
            try {
                people.enqueue("Person " + DEFAULT_BPQ_CAPACITY + i, priority);
            } catch(Throwable t){
                fail("After inserting person #" + DEFAULT_BPQ_CAPACITY + i +
            ...
            assertEquals("Inserting elements beyond a BPQ's capacity should not expand the queue, "
                    ...
                    DEFAULT_BPQ_CAPACITY + i + "\"" + " with priority " +
                    priority + ".",	people.size(), DEFAULT_BPQ_CAPACITY);
        }
    }

    @Test
    public void testBPQDequeue() {

        // (1) An easy test.
        people.enqueue("Jim", r.nextInt(MAX_ELEMENT_PRIORITY));
        ...
        Double[] priorities = new Double[DEFAULT_BPQ_CAPACITY];
    ...
    public void testBPQFirst() {
        ...
        assertFalse("A singleton BPQ should not be empty after peeking its top element.",
                people.isEmpty());
        people = new BoundedPriorityQueue<>(DEFAULT_BPQ_CAPACITY); // reset queue
        // (2) Make sure that two elements with equal priorities
        // get processed in a FIFO manner.
        people.enqueue("Jim", 11.3);
        people.enqueue("Jill", 11.3);
        ...
        assertEquals("Inserting a minimum priority element after the queue is full should not alter its size.",
                DEFAULT_BPQ_CAPACITY, people.size());
    ...
    @Test
    public void testBPQFailFastIterator(){
	...
	/*@Test
	public void testBPQSize() {
		fail("Not yet implemented");
	}

	@Test
	public void testBPQIsEmpty() {
		fail("Not yet implemented");
	}
     ...
     */
    @Test
    public void testKDTreeDefConstructor() {
        for(int i = 0; i < MAX_KDTREE_ITER; i++){
            try {
                new KDTree();
            } catch(Throwable t){
     ...
     */
    @Test
    public void testKDTreeIntConstructor() {
    ...
    /**
     * Test method for {@link KDTree#insert(utils.KDPoint)}.
     */
    @Test
    public void testKDTreeInsert() {
        ...
        for(int dim = 1; dim < MAX_DIM + 1; dim++){ // For all possible dimensions that we consider...
            KDTree tempTree = new KDTree(dim);
            for(int i = 0; i < MAX_KDTREE_ITER; i++){ // Bunch of points
    ...
    /**
     * Test method for {@link KDTree#delete(utils.KDPoint)}.
     */
    @Test
    public void testKDTreeDelete() {
		 ...
		 *	MAX_KDTREE_ITER-many KDPoints of the appropriate dimensionality, while
        ...
        for(int dim = 1; dim < MAX_DIM + 1; dim++){ // For MAX_DIM-many trees...
            ...
            for(int i = 0; i < MAX_KDTREE_ITER; i++){ // For MAX_KDTREE_ITER-many points...
    ...
    /**
     * Test method for {@link KDTree#search(utils.KDPoint)}.
     */
    @Test
    ...
    /**
     * Test method for {@link KDTree#range(utils.KDPoint, double)}.
     */
    @Test
    ...
    /**
     * Test method for {@link KDTree#nearestNeighbor(utils.KDPoint)}.
     */
    @Test
    public void testKDTreeNearestNeighbor() {
        ...
        // gap between them, check nearest neighbor for every one. Repeat this MAX_KDTREE_ITER-many times.
    ...
    /**
     * Test method for {@link KDTree#kNearestNeighbors(int, utils.KDPoint)}.
     */
    @Test
    public void testKDTreeKNearestNeighbors() {
        ...
        for(int dim = 1; dim <= MAX_DIM; dim++){
            ...
            // And then we check if all possible k-nn queries, are satisfied.
            for(int k = 1; k < MAX_KDTREE_ITER; k++)
